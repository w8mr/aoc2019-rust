...
mod tests {
    ...
    fn test_day5_part1_opcode030499() {
        let outputs = intcode::run_input_output(&vec!(3, 0, 4, 0, 99), &vec!(42));
    ...
    fn test_day5_part2_example1_not_equal() {
        let outputs = intcode::run_input_output(&vec!(3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8), &vec!(5));
        assert_eq!(outputs, vec!(0));
    }

    #[test]
    fn test_day5_part2_example1_equal() {
        let outputs = intcode::run_input_output(&vec!(3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8), &vec!(8));
        assert_eq!(outputs, vec!(1));
    }

    #[test]
    fn test_day5_part2_example4_less_than() {
        let outputs = intcode::run_input_output(&vec!(3, 3, 1107, -1, 8, 3, 4, 3, 99), &vec!(5));
        assert_eq!(outputs, vec!(1));
    }

    #[test]
    fn test_day5_part2_example4_not_less_than() {
        let outputs = intcode::run_input_output(&vec!(3, 3, 1107, -1, 8, 3, 4, 3, 99), &vec!(8));
        assert_eq!(outputs, vec!(0));
    }

    #[test]
    fn test_day5_part2_jmp_example1_jmp() {
        let outputs = intcode::run_input_output(&vec!(3, 12, 6, 12, 15, 1, 13, 14, 13, 4, 13, 99, -1, 0, 1, 9), &vec!(0));
        assert_eq!(outputs, vec!(0));
    }

    #[test]
    fn test_day5_part2_large_example4_less_than() {
        let outputs = intcode::run_input_output(&vec!(
