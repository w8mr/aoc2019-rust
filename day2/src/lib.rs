...
struct Instruction {
    opcode: Opcode,
    operand_count: usize,
    implementation: fn(Vec<isize>, Vec<isize>, &mut Context) -> Option<i32>,
}
...
fn add_implementation(parameters: Vec<isize>, modes: Vec<isize>, context: &mut Context) -> Option<i32> {
    context.memory[parameters[2] as usize] = read(&parameters, &modes, context, 0) + read(&parameters, &modes, context, 1);
    Some(4)
}


fn multiply_implementation(parameters: Vec<isize>, modes: Vec<isize>, context: &mut Context) -> Option<i32>  {
    context.memory[parameters[2] as usize] = read(&parameters, &modes, context, 0) * read(&parameters, &modes, context, 1);
    Some(4)
}

fn input_implementation(parameters: Vec<isize>, modes: Vec<isize>, context: &mut Context) -> Option<i32>  {
    context.memory[parameters[0] as usize] = context.inputs.pop().unwrap();
    Some(2)
}

fn output_implementation(parameters: Vec<isize>, modes: Vec<isize>, context: &mut Context) -> Option<i32>  {
    context.outputs.push(read(&parameters, &modes, context, 0));
    Some(2)
}


fn halt_implementation(parameters: Vec<isize>, modes: Vec<isize>, context: &mut Context) -> Option<i32>  {
    None
}
...
pub fn day5(opcodes: &Vec<isize>, inputs: &Vec<isize>, outputs: &mut Vec<isize>) -> isize {
    ...
    loop {
        ...
        let parameters = ((offset as usize + 1)..(offset as usize + 1 + instruction.operand_count)).map(|i| context.memory[i]).collect();
